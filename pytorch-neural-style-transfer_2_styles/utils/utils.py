...
def generate_out_img_name(config, two_styles):
    if two_styles:
        prefix = config['content_img_name'].split('.')[0] + '_' + config['style_img_name'].split('.')[0] + '_' + config['style_img2_name'].split('.')[0]
    else:
    ...
    if 'reconstruct_script' in config:
        ...
        if two_styles:
            suffix = f'_o_{config["optimizer"]}_i_{config["init_method"]}_h_{str(config["height"])}_m_{config["model"]}_cw_{config["content_weight"]}_sw_{config["style_weight"]}_s2w_{config["style2_weight"]}_tv_{config["tv_weight"]}{config["img_format"][1]}'
        else:
...
def save_and_maybe_display(optimizing_img, dump_path, config, img_id, num_of_iterations, two_styles, should_display=False):
    ...
    if img_id == num_of_iterations-1 or (saving_freq > 0 and img_id % saving_freq == 0):
        ...
        out_img_name = str(img_id).zfill(img_format[0]) + img_format[1] if saving_freq != -1 else generate_out_img_name(config, two_styles)
